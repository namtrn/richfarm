export function selectReminderRows<T>(
    ...
    // A complete projection is authoritative even when it is empty. During
    // hydration, merge the partial projection over fallback rows so rows that
    // have not arrived yet remain visible.
...
export function isReminderSnoozed(reminder: { snoozedUntil?: number } | null | undefined, now: number) {
