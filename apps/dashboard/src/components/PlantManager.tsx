...
function getFamilyLabel(plant: PlantHook["plants"][number]) {
    const family = String(plant.family ?? "").trim();
    return family || "Unassigned family";
}

function getCommonGenusName(plant: PlantHook["plants"][number], locale: "vi" | "en" = "vi") {
    return locale === "vi"
        ? plant.commonGenusNameVi || plant.commonGenusNameEn || ""
        : plant.commonGenusNameEn || plant.commonGenusNameVi || "";
}

function getCommonSpeciesName(plant: PlantHook["plants"][number], locale: "vi" | "en" = "vi") {
    return locale === "vi"
        ? plant.commonSpeciesNameVi || plant.commonSpeciesNameEn || ""
        : plant.commonSpeciesNameEn || plant.commonSpeciesNameVi || "";
}
...
function getUiGroupRows(plants: PlantHook["plants"]) {
    ...
    for (const group of buildSortedPlantUiClusters(plants, "vi")) {
        rows.push({
            rowType: "header",
...
function getFamilyGroupRows(plants: PlantHook["plants"]) {
    const grouped = new Map<string, PlantHook["plants"]>();

    for (const plant of plants) {
        const family = getFamilyLabel(plant);
        const existing = grouped.get(family);
        if (existing) {
            existing.push(plant);
        } else {
            grouped.set(family, [plant]);
        }
    }

    const rows: Array<
        | { rowType: "family-header"; key: string; label: string; count: number }
        | { rowType: "plant"; key: string; plant: PlantHook["plants"][number] }
    > = [];

    for (const [family, familyPlants] of Array.from(grouped.entries()).sort((a, b) =>
        a[0].localeCompare(b[0], "vi")
    )) {
        rows.push({
            rowType: "family-header",
            key: `family:${family}`,
            label: family,
...
export function PlantManager({
    ...
    const groupedRows = useMemo(
        () => (p.viewMode === "family" ? getFamilyGroupRows(p.plants) : getUiGroupRows(p.plants)),
        [p.plants, p.viewMode],
    );
    ...
    return (
        <div className="page-content">
            ...
            <div className="layout">
                ...
                <section className="card">
                    ...
                    <div className="filters">
                        <input
                            ...
                            placeholder="🔍 Search by name, genus, species, cultivar, category..."
                            value={p.search}
                    ...
                    <div className="form-tabs" style={{ marginTop: "0.75rem", marginBottom: "0.9rem" }}>
                        <button
                            type="button"
                            className={`form-tab ${p.viewMode === "common" ? "active" : ""}`}
                            onClick={() => p.setViewMode("common")}
                        >
                            Genus Clusters
                        </button>
                        <button
                            type="button"
                            className={`form-tab ${p.viewMode === "family" ? "active" : ""}`}
                            onClick={() => p.setViewMode("family")}
                        >
                            Family Clusters
                        </button>
                    </div>
                    ...
                    <div className="actions" style={{ justifyContent: "space-between", marginTop: "1rem" }}>
                        <span className="muted">30 plants per page. Taxonomy clusters stay on one page.</span>
                        <div className="actions">
                ...
                {/* Right: detail / form */}
                <section className="card" style={{ position: "sticky", top: 24, alignSelf: "start", maxHeight: "calc(100vh - 48px)", overflowY: "auto" }}>
                    <div className="section-title">
                        <h3>
...
function PlantDetail({
    ...
    const variant = isVariantRow(plant.cultivarNormalized);
    const genusCommonName = getCommonGenusName(plant);
    const speciesCommonName = getCommonSpeciesName(plant);
            ...
            <div className="detail-header">
                <div>
                    ...
                    <p className="muted" style={{ fontSize: "0.8rem" }}>
                        Family: {getFamilyLabel(plant)}
                    </p>
                    {genusCommonName && (
                        <p className="muted" style={{ fontSize: "0.8rem" }}>
                            Common genus name: {genusCommonName}
                        </p>
                    )}
                    {speciesCommonName && (
                        <p className="muted" style={{ fontSize: "0.8rem" }}>
                            Common species name: {speciesCommonName}
                        </p>
                    )}
                    {plant.genus && (
                        <p className="muted" style={{ fontSize: "0.8rem" }}>
...
function PlantForm({
    ...
    return (
        <div className="form simplified-form">
            ...
            <div className="form-section">
                <div className="section-header-compact">🏷 Taxonomy Common Names</div>
                <p className="form-preview muted">
                    Shared labels for the whole genus/species, not just this plant row.
                </p>
                <div className="grid-2">
                    <label>
                        Common genus name (VI)
                        <input placeholder="e.g. Mon kieng" value={f.commonGenusNameVi} onChange={(e) => set({ commonGenusNameVi: e.target.value })} />
                    </label>
                    <label>
                        Common genus name (EN)
                        <input placeholder="e.g. Alocasia" value={f.commonGenusNameEn} onChange={(e) => set({ commonGenusNameEn: e.target.value })} />
                    </label>
                    <label>
                        Common species name (VI)
                        <input placeholder="e.g. Mon kieng Amazon" value={f.commonSpeciesNameVi} onChange={(e) => set({ commonSpeciesNameVi: e.target.value })} />
                    </label>
                    <label>
                        Common species name (EN)
                        <input placeholder="e.g. Amazonica" value={f.commonSpeciesNameEn} onChange={(e) => set({ commonSpeciesNameEn: e.target.value })} />
                    </label>
                </div>
            </div>
            ...
            <div className="form-section">
                ...
                <div className="grid-3">
                    <label>
                        Base Species Plant ID
                        <input placeholder="Convex plant id" value={f.basePlantId} onChange={(e) => set({ basePlantId: e.target.value })} />
                    </label>
                    <label>
                        Common Name Group Key
                        <input placeholder="e.g. hanh-tay" value={f.commonNameGroupKey} onChange={(e) => set({ commonNameGroupKey: e.target.value })} />
                    </label>
                    <label>
                        Common Name Group (VI)
                        <input placeholder="e.g. Hanh tay" value={f.commonNameGroupVi} onChange={(e) => set({ commonNameGroupVi: e.target.value })} />
                    </label>
                    <label>
                        Common Name Group (EN)
                        <input placeholder="e.g. Onion" value={f.commonNameGroupEn} onChange={(e) => set({ commonNameGroupEn: e.target.value })} />
                    </label>
                </div>
                <p className="form-preview muted">
                    Optional override for grouping variants under a shared common-name cluster.
                </p>
                <label>
                    Image URL
