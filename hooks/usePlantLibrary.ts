...
export function usePlantLibrary(
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                setCachedPlants(Array.isArray(parsedPlants) ? parsedPlants : null);
                setCachedPlantsWithoutImages(Array.isArray(parsedWithout) ? parsedWithout : null);
            } catch {
                if (cancelled) return;
    ...
    useEffect(() => {
        ...
        setCachedPlants(remotePlants);
        AsyncStorage.setItem(
            plantsCacheKey(normalizedLocale),
            JSON.stringify(remotePlants)
        ).catch(() => undefined);
    }, [remotePlants, normalizedLocale]);

    useEffect(() => {
        if (!Array.isArray(remotePlantsWithoutImages)) return;
        setCachedPlantsWithoutImages(remotePlantsWithoutImages);
        AsyncStorage.setItem(
            plantsWithoutImagesCacheKey(normalizedLocale),
            JSON.stringify(remotePlantsWithoutImages)
        ).catch(() => undefined);
    }, [remotePlantsWithoutImages, normalizedLocale]);
    ...
    const shouldUseSeed =
        ...
        !hasCached;
    const plants =
        (Array.isArray(remotePlants) ? remotePlants : null) ??
        cachedPlants ??
        (shouldUseSeed ? seedPlants : []);
    const plantsWithoutImages =
        (Array.isArray(remotePlantsWithoutImages)
