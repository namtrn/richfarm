...
export default function SpeciesScreen() {
    ...
    const speciesNormalized = Array.isArray(params.speciesNormalized) ? params.speciesNormalized[0] : params.speciesNormalized;
    ...
    const plants = useQuery(
        api.plantLibrary.listPlantsBySpecies,
        genusNormalized && speciesNormalized
            ? { family, genusNormalized, speciesNormalized, locale, limit: 200 }
            : 'skip'
    );
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((item: any) =>
            matchesSearch(query, [item.commonName, item.scientificName, item.cultivar])
        );
    }, [plants, search]);
            ...
            <View style={{ paddingHorizontal: 16, paddingTop: 14, paddingBottom: 12, gap: 12, backgroundColor: theme.card, borderBottomWidth: 1, borderBottomColor: theme.border }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ fontSize: 12, color: theme.textSecondary }}>{family ?? 'Family'} • Plant entries</Text>
                    </View>
                </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }: { item: any }) => {
                            ...
                            return (
                                ...
                                >
                                    <View style={{ height: 110, width: '100%', backgroundColor: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
                                        <PlantImage uri={item.imageUrl} size="100%" borderRadius={0} />
                                    </View>
                        ...
                        return (
                            ...
                            >
                                <PlantImage uri={item.imageUrl ?? undefined} size={52} borderRadius={14} />
