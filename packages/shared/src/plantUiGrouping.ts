...
function getLocalizedGenusCommonName(plant: any, locale?: string) {
  const normalizedLocale = String(locale ?? "").split("-")[0]?.toLowerCase();
  const vi = String(plant?.commonGenusNameVi ?? "").trim();
  const en = String(plant?.commonGenusNameEn ?? "").trim();
  return normalizedLocale === "vi" ? vi || en : en || vi;
}

function getLocalizedSpeciesCommonName(plant: any, locale?: string) {
  const normalizedLocale = String(locale ?? "").split("-")[0]?.toLowerCase();
  const vi = String(plant?.commonSpeciesNameVi ?? "").trim();
  const en = String(plant?.commonSpeciesNameEn ?? "").trim();
  return normalizedLocale === "vi" ? vi || en : en || vi;
}
...
function buildFallbackGroup(plant: any, locale?: string) {
  const label = String(
    getLocalizedSpeciesCommonName(plant, locale) ||
      getLocalizedName(plant, locale) ||
      "Plant"
  ).trim();
  return {
...
function getExplicitGroup(plant: any, locale?: string) {
  const normalizedLocale = String(locale ?? "").split("-")[0]?.toLowerCase();
  const key = String(plant?.commonNameGroupKey ?? "").trim();
  const labelVi = String(plant?.commonNameGroupVi ?? "").trim();
  const labelEn = String(plant?.commonNameGroupEn ?? "").trim();
  const label =
    normalizedLocale === "vi"
      ? labelVi || labelEn
      : labelEn || labelVi;
...
function buildGenusGroup(plant: any, locale?: string) {
  const genusNormalized = normalizeGroupingText(
    plant?.genus ?? ""
  );
  const genus = String(plant?.genus ?? "").trim();
  if (!genusNormalized || !genus) return null;

  return {
    key: `genus:${genusNormalized}`,
    label: getLocalizedGenusCommonName(plant, locale) || genus,
  };
}
...
export function buildPlantUiGroupMap(plants: any[], locale?: string) {
  ...
  return new Map(
    plants.map((plant) => {
      ...
      const genusGroup = buildGenusGroup(plant, locale);
      if (genusGroup) {
        return [String(plant?._id), genusGroup];
      }

      const basePlantId = String(plant?.basePlantId ?? "").trim();
      if (basePlantId) {
        const basePlant = plantsById.get(basePlantId) ?? (String(plant?._id) === basePlantId ? plant : null);
        if (basePlant) {
          return [
            String(plant?._id),
            {
