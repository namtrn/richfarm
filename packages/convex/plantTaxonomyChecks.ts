...
function buildDbTaxonomyKey(plant: any) {
  ...
  return taxonomyIdentityKey({
    genusNormalized: identity.genusNormalized,
    speciesNormalized: identity.speciesNormalized,
    cultivarNormalized: identity.cultivarNormalized,
  });
}
...
async function buildInvariantReport(ctx: any, sampleLimit: number) {
  ...
  for (const plant of plants) {
    ...
    if (!key) continue;
    const identity = taxonomyIdentityFromPlant(plant);
    if (identity.cultivarNormalized === DEFAULT_CULTIVAR_NORMALIZED) {
      continue;
    }
    if (isInfraspecificCultivar(identity.cultivarNormalized)) {
      continue;
    }
...
async function buildFamilyReport(ctx: any, sampleLimit: number) {
  ...
  for (const plant of plants) {
    const genusNormalized = normalizeTaxonomyToken(String(plant?.genus ?? ""));
