...
- `apps/api/src/content-source/`: content change detection state (MCD-1/MCD-2). `contract.ts` pins roots, lease/SLA/audit/retention constants, review state machine, and legacy-baseline owner-status rules; `paths.ts` owns path normalization/classification and case-folded collision keys; `schema.ts` is the additive SQLite migration wired into `createDatabase`; `repository.ts` provides the idempotent file index, durable event journal (coalescing/supersession/rename correlation), per-shard checkpoints, quarantine retry, monitor lease, scoped proposal watermarks, and health-only global revision; `retention.ts` implements configurable 90-day/512-MiB compaction (disabled until backup/rebuild proof) plus SQLite file backup.
- `apps/api/src/content-source/scanner.ts`, `watcher.ts`, `monitor.ts`: detection runtime (MCD-3/MCD-4). The scanner performs bounded per-shard reconciliation with a vanished-shard deletion sweep, manifest-neighborhood revalidation, atomic one-time legacy baseline, and a budgeted/resumable full-hash audit window with persisted per-root cursors; `processCandidatePath` is the single shared indexing entry for watcher candidates (file + manifest neighborhood only). `watcher.ts` wraps chokidar v4 (pinned `4.0.3`, no native build) behind a narrow adapter plus a bounded per-path debounce queue whose overflow flushes instead of dropping. `monitor.ts` owns lifecycle: baseline → catch-up → lease-gated watching (one active instance per root, expiry takeover, renewal every 10s), periodic reconcile, hourly audit ticks, graceful stop that closes handles before SQLite, and a health snapshot (`ready/degraded/passive/disabled`) with degraded reasons. Wired into `server.ts` after DB init; disabled via `CONTENT_SOURCE_MONITOR_ENABLED=false`.
